...
# ==========================================
# CẤU HÌNH HÌNH HỌC CAMERA (THEO Ý TƯỞNG CỦA CÔ)
# ==========================================
...
while True:
    ...
    # Danh sách lưu thông tin hướng phục vụ việc đọc loa
    ...
    for obj in detections:
        ...
        # --- [TÍNH TOÁN THEO Ý CÔ] XỬ LÝ LƯỢNG GIÁC GÓC LỆCH BETA ---
        x_center = (x1 + x2) / 2
        # Tính toán góc Beta lệch so với tâm phân giác (Chính diện)
        beta_angle = ((x_center - FRAME_CENTER_X) / FRAME_CENTER_X) * (FOV_HORIZONTAL / 2)
        
        # Phân loại hướng không gian dựa trên góc Beta
        if beta_angle > 8:
        ...
        # Lưu thông tin vật thể để lát chọn đọc ra loa
        ...
        # Vẽ hộp nhận diện lên màn hình debug (Hiển thị luôn góc lệch deg)
        ...
        if total_objects > 0:
            ...
            if current_time - last_audio_time > 5:
                if len(detected_objects_info) > 0:
                    # Lấy vật thể đầu tiên quét được để thông báo hướng
                    ...
                    # Hệ thống sẽ đọc cực thông minh: "Chú ý. Có người bên trái."
    ...
    key = cv2.waitKey(1)
    time.sleep(0.005) # Giải phóng băng thông CPU chống sập nguồn cổng USB
