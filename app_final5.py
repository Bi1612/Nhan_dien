...
while True:
    ...
    # Lấy dữ liệu khoảng cách mới nhất từ luồng ngoại vi Arduino
    current_physical_distance = shared.distance 
    
    # [BỔ SUNG ĐỒ HỌA]: Khởi tạo mặc định nguồn dữ liệu từ cảm biến siêu âm
    sensor_source = "SONAR"
    ...
    # --- ĐIỀU KIỆN 1: CẢNH BÁO DỰA TRÊN CẢM BIẾN SIÊU ÂM TRONG VÙNG NGUY HIỂM ---
    ...
    if (current_physical_distance == 999.0 or current_physical_distance is None) and len(detections) > 0:
        ...
        # Thuật toán bù dữ liệu bằng mô hình lỗ kim hình học camera của Bi
        current_physical_distance = (180 / max(y2_f - y1_f, 1)) * 150.0
        # [BỔ SUNG ĐỒ HỌA]: Kích hoạt cờ trạng thái lỗi, chuyển sang nguồn Camera dự phòng
        sensor_source = "WEBCAM"
    ...
    for obj in detections:
        ...
        # --- THUẬT TOÁN CỦA CÔ THẢO: TÍNH GÓC LỆCH BETA PHƯƠNG VỊ ---
        x_center = (x1 + x2) / 2
    ...
    # ==========================================
    H, W, _ = frame.shape
    # Thiết lập hộp Dashboard màu đen (Nới rộng chiều cao để chứa nhãn dự phòng)
    ...
    # [BỔ SUNG ĐỒ HỌA]: Điều khiển màu sắc số hiển thị theo nguồn dữ liệu
    if sensor_source == "WEBCAM":
        dist_color = (0, 255, 255) # Màu vàng rực cảnh báo chế độ ước lượng hình học
        # Hiển thị nhãn phụ tĩnh/nhấp nháy minh bạch hóa luồng chịu lỗi cảm biến
        cv2.putText(frame, "[CAMERA EST]", (panel_text_x, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 255), 1)
    else:
        dist_color = (0, 0, 255) if current_physical_distance < SENSOR_DANGER_THRESHOLD_CM else (255, 255, 255)
        
    cv2.putText(frame, f"Dist: {current_physical_distance:.1f} cm", (panel_text_x, 65), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, dist_color, 1)
    ...
    # Đẩy vị trí hiển thị các đối tượng nhận diện xuống dưới nhường chỗ cho nhãn cảnh báo
    cv2.putText(frame, f"P: {counts['person']}", (panel_text_x, 175), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (0, 0, 255), 1)
    cv2.putText(frame, f"C: {counts['car']}", (panel_text_x, 195), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (255, 0, 0), 1)
    cv2.putText(frame, f"M: {counts['motorcycle']}", (panel_text_x, 215), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (0, 255, 255), 1)
    cv2.putText(frame, f"B: {counts['bus']}", (panel_text_x, 235), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (0, 255, 0), 1)
    cv2.putText(frame, f"T: {counts['truck']}", (panel_text_x, 255), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (255, 0, 255), 1)
    
    cv2.putText(frame, "SYSTEM STATUS:", (panel_text_x, 290), cv2.FONT_HERSHEY_SIMPLEX, panel_font_scale, (255, 255, 255), 1)
    cv2.putText(frame, status, (panel_text_x, 315), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2)
    cv2.putText(frame, status_long, (panel_text_x, 340), cv2.FONT_HERSHEY_SIMPLEX, 0.4, status_color, 1)
